<template>
  <div id="app">
    <div style="background-image: url(&#39;https://s2.ax1x.com/2020/01/24/1ZkT7F.md.jpg&#39;); margin-bottom: 10px">
      <div class="hero-inner">
        <h2 style="color: #fff">物资需求将在这里整合</h2>
        <p style="color: #fff">现在武汉的一线医院物资短缺，急需：N95 口罩、护目镜、防护服等。请大家有医用防护物资来源渠道请联系以下电话</p>
        <van-button style="margin-bottom: 12px; background-color: #000; boder: 1px solid #000;cursor: pointer">
    ...
    <!-- <van-list v-model="loading" finished = "finished" finished-text="没有更多了" @load="onLoad"> -->
      <div v-for="content in list" :key="content.name">
        <!-- 医院名称 -->
        <van-cell style="font-size: 18px; background-color: rgb(183, 136, 182)"
          :title="content.name"
        />
        <!-- 物资列表 -->
        <van-cell title="物资名称" value="物资数量"></van-cell>
        <div v-for="item in content.items" :key="item.name" style="border: 1px solid #000010; margin: 20px; font-size: 10px">
          <van-cell
            :title="item.name"
            :value="item.amount"
          />
